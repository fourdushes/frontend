...
| 피보호자 | 의료기관 검색, 진료 요청·대기 요청 취소, 수락 시 진료방 자동 입장, 텍스트 대화, 진료 종료, AI 요약 아카이브 조회, 보호자 연결 승인·거절 |
| 보호자 | 피보호자 검색·연결 신청, 연결 관리, 승인된 피보호자의 진료 아카이브 조회 |
| 기관 소속 사용자 | 도착한 진료 요청 수락·거절, 수락 시 진료방 즉시 입장, 자동 발화 녹음 및 STT 메시지 전송 |
| 기관 관리자 | 기관 전용 로그인, 소속 사용자 상태별 현황과 이름·ID 검색, 가입 요청 승인·거절, 승인 사용자 연결 해제 |

일반 사용자 JWT 세션은 `AsyncStorage`에 보관하고, API 요청에 Access Token을 자동 첨부합니다. HTTP 401 응답 시 Refresh Token으로 토큰을 한 번 갱신한 뒤 원래 요청을 재시도합니다. 기관 관리자 세션은 일반 사용자 세션과 별도의 저장소 키로 관리합니다. 진료 요청과 진료방의 연결 정보는 로컬에 저장하지 않고 백엔드가 반환하는 `chatRoomId`, `archiveId`를 기준으로 복구합니다.
...
│   ├── audio/              # 자동 발화 감지, 음성 조각 생성·전송 큐
│   ├── components/         # 공통 UI
│   ├── context/            # 로그인 세션, 진행 중인 진료 요청 상태
│   ├── screens/            # 역할별 화면
│   ├── theme/              # 색상·타이포그래피
│   ├── types/              # API 응답 타입
│   └── navigation.tsx
...
- 기관 회원가입과 기관 아이디 찾기·비밀번호 변경은 기관 전용 API를 호출하며, 일반 사용자 계정 복구 흐름과 분리되어 있습니다.
- 기관 관리자 화면은 승인 대기·승인·거절·삭제 사용자를 페이지 단위로 조회하고, 이름 또는 사용자 ID를 서버에서 검색하며 승인·거절·연결 해제를 처리합니다.
- 피보호자는 기관 사용자를 검색해 진료를 요청하고, `REQUESTED` 상태에서는 같은 화면에서 요청을 취소할 수 있습니다.
- 진료 요청 응답의 `chatRoomId`, `archiveId`를 이용해 기관 사용자는 수락 직후, 피보호자는 요청 상태 동기화 후 같은 진료방에 자동 입장합니다.
- 활성 진료 요청은 화면 표시 상태에서 3초, 백그라운드에서 12초 간격으로 갱신하고 화면이 다시 활성화되면 즉시 동기화합니다. 요청 대기 중에는 진료 요청 화면을 유지하고 진료 중에는 완료 전까지 해당 진료방으로 복귀시킵니다.
- Web 진료방은 주변 소음을 보정해 발화를 감지하고, 2.5초 침묵 시 16kHz mono WAV 조각을 순서대로 전송합니다. 일시적인 전송 실패는 자동 재시도하며 실패 조각은 사용자가 다시 보낼 수 있습니다.
...
- 기관 자체 회원가입과 기관 아이디 찾기·비밀번호 변경 화면이 기관 API에 연결되어 있습니다.
- 기관 관리자 검색은 `/api/institutions/search/institution-user`에 검색어·상태·페이지를 전달하며, 입력을 멈춘 뒤 자동으로 결과를 갱신합니다.
- 보호자는 진료 대화에 직접 참여하지 않고 승인된 피보호자의 완료 아카이브를 조회합니다.
- 기관 소속 사용자 계정의 아카이브 목록은 현재 제공하지 않습니다.
- 피보호자의 진료 요청 취소 버튼은 `/api/medical-treatment/ward/requests/{requestId}/cancel` API와 연결되어 있습니다.
- 진료 요청과 대화방 연결은 백엔드 응답의 `chatRoomId`, `archiveId`만 사용하므로 브라우저나 기기가 달라도 같은 계정의 진행 중 진료를 다시 확인할 수 있습니다.
- 응답 대기 중에는 다른 화면으로 이동할 수 없으며, 요청 취소 또는 기관 사용자의 거절 이후에는 새로운 기관 사용자에게 다시 요청할 수 있습니다.
- 자동 발화 녹음은 Web Audio API를 지원하는 브라우저에서 제공하며, 그 외 환경에서는 수동 녹음 방식을 사용합니다.
