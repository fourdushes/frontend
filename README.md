...
| 피보호자 | 의료기관 검색, 진료 요청 상태 확인, 수락 시 진료방 자동 입장, 텍스트 대화, 진료 종료, AI 요약 아카이브 조회, 보호자 연결 승인·거절 |
...
- 활성 진료 요청은 화면 표시 상태에서 3초 간격으로 갱신하며, 요청 대기 중에는 진료 요청 화면을 유지하고 진료 중에는 완료 전까지 해당 진료방으로 복귀시킵니다.
...
- 진료 요청 취소 UI는 준비되어 있지만 실제 취소는 백엔드 취소 API가 추가된 뒤 연결해야 합니다.
- 진료 요청과 대화방 연결은 백엔드 응답의 `chatRoomId`를 우선 사용하며, 기존 요청 호환을 위해 로컬 저장소 값을 보조적으로 사용합니다.
